defmodule Nc.Servers.ClientServer do
  ...
  @spec transform_incoming(DocServer.change(), [DocServer.change()]) :: {[DocServer.change()] | DocServer.change() | nil, [DocServer.change()]}
    ...
    case pending_changes do
      [pending_change | pending_changes] ->
        ...
        case localized_change do
          nil ->
            new_pending_change = Nc.Sync.Transforms.transform_outgoing(pending_change, incoming_change)
            {nil, [new_pending_change | pending_changes]}
          {_,_,_} ->
            new_pending_change = Nc.Sync.Transforms.transform_outgoing(pending_change, incoming_change)
            {new_localized_change, pending_changes} = transform_incoming(localized_change, pending_changes)
  ...
  @spec localize_incoming([DocServer.change()], [DocServer.change()]) :: {[DocServer.change()], [DocServer.change()]}
  def localize_incoming(incoming_changes, pending_changes) do
    case incoming_changes do
      [incoming | incoming_rest] ->
        ...
        localized = transform_incoming(incoming, pending_changes) |> IO.inspect(label: "localized")
        pending = Enum.map(
          pending_changes,
          fn pending_change ->
        ...
        case localized do
          nil -> localize_incoming(incoming_rest, pending)
          {_,_,_} ->
  ...
  def handle_pull(state) do
    ...
    {incoming_changes, current_id} = collect_change_messages() |> IO.inspect(label: "incoming changes")
    ...
    if current_id == -1 do
      ...
      localized_incoming_changes = localize_incoming(incoming_changes, Enum.reverse(pending_changes)) |> IO.inspect(label: "localized changes")
